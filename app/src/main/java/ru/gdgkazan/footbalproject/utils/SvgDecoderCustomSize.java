...
public class SvgDecoderCustomSize implements ResourceDecoder<InputStream, SVG> {

    public Resource<SVG> decode(InputStream source, int width, int height) throws IOException {
        try {
            SVG svg = SVG.getFromInputStream(source);

            svg.setDocumentWidth(Images.IMAGE_SIZE);
            svg.setDocumentHeight(Images.IMAGE_SIZE);
